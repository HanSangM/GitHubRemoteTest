...
<div class="container">
	...
	<fieldset>
			  ...
			  public String 메소드(@RequestParam 자료형 매개변수){
    			...
    			@RequestParam("파라미터명") 자료형 매개변수명
     			- 해당 파라미터의 자료형으로 받을 수 있다
             	    즉 형변환 불필요
     			- 파라미터가 1~2개 정도일때 유리 (여러개일때는 자료형을 Map으로)
     			-->
		...
		<hr />
		<fieldset>
			<legend>@Lazy 어노테이션(지연로딩을 위한 어노테이션) ${message !=null ? message :""}</legend>
		...
		<fieldset>
			...
			<h4>
				<h5>@RequestBody : JSON데이터</h5>
				<a href="javascript:sendJson()">JSON데이터</a>
				<h5>@RequestBody : FORM데이터</h5>
...
<script>
	...
	function sendJson(){
		...
		}).done(function(data){//위에 있는 data가 아니고 서버로 부터 받은 data 이다.
			console.log(data)
